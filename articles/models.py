...
def image_filename(instance, filename):
    if instance.image_uuid == "" or instance.image_uuid is None:
        instance.image_uuid = uuid.uuid4().hex
    return "uploads/post/%s" % (instance.image_uuid,)
...
class Article(models.Model):
    ...
    image = models.ImageField(upload_to=image_filename, storage=OverwriteImageStorage(), blank=True)
    image_uuid = models.CharField(max_length=32, unique=True, null=True, blank=True)
    ...
    def do_unique_slug(self, using=DEFAULT_DB):
        ...
        if not self.id:
            ...
            if not len(self.slug.strip()):
                self.slug = slugify(self.title.lower().replace(u'ä', u'ae').replace(u'ü', u'ue').replace(u'ö', u'oe'))
